    ...
    <main class="main">
        ...
        <?php else : ?>
            ...
            <section class="section__profile">
                <article class="article__image">
                    <img src="web/images/profile.png" alt="" class="img__profile">
                </article>
                <article class="article__profile">
                    <form action="index.php?accion=editProfile&id=<?= $user->getId() ?>" method="POST" class="form__profile">
                        <label for="username" class="label__login">Nombre Usuario</label><br>
                        <input type="text" name="username" class="input__login w-75" value="<?= $user->getUserName() ?>"><br>
                        <label for="email" class="label__login">Correo Electrónico</label><br>
                        <input type="text" name="email" class="input__login w-75" value="<?= $user->getEmail() ?>"><br>
                        <label for="firstName" class="label__login">Nombre</label><br>
                        <input type="text" name="firstName" class="input__login w-75" value="<?= $user->getFirstName() ?>"><br>
                        <label for="lastName" class="label__login">Apellidos</label><br>
                        <input type="text" name="lastName" class="input__login w-75" value="<?= $user->getLastName() ?>"><br>
                        <label for="dni" class="label__login">DNI</label><br>
                        <input type="text" name="dni" class="input__login w-75" value="<?= $user->getDni() ?>"><br>
                        <div class="form__button">
                            <input type="submit" class="button__login" value="GUARDAR">
                            <button class="button__login"><a href="index.php?accion=inicio" class="link__login">VOLVER</a></button>
                        </div>
                    </form>
                </article>
            </section>
